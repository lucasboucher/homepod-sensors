...
- `ROOMS_FILE_HOST` — absolute host path to your local `rooms.json`

`PAIRING_FILE` inside the container is always `/app/pairing.json`. You do not
need to set it. `ROOMS_FILE` is always `/app/rooms.json`. Compose mounts
`${ROOMS_FILE_HOST}` at `/app/rooms.json:ro`; [rooms.example.json](collector/rooms.example.json)
is documentation only and is not mounted automatically.
If `/app/rooms.json` is missing or unreadable, the collector still starts and
falls back to HAP Name, then the pairing alias.
Optional poll/timeout variables default to 600 / 15 / 10 seconds if omitted.
The default poll interval is 10 minutes; the collector waits for the next
clock-aligned multiple of that interval (HH:00, HH:10, HH:20, HH:30, HH:40,
HH:50 in UTC) instead of sleeping 10 minutes after each cycle. Other intervals
that divide an hour the same way stay clock-aligned.

```bash
cd collector
cp env.example .env
# edit .env: PAIRING_FILE_HOST, ROOMS_FILE_HOST, and HTTP_ENDPOINT_URL (fictitious values in the example)
...
`/app/pairing.json:ro` and `${ROOMS_FILE_HOST}` at `/app/rooms.json:ro`.
Do not bake `pairing.json`, `rooms.json`, or the endpoint URL into the image.
...
- `name` comes from local `rooms.json` when mapped, else HAP Name, else the pairing alias
